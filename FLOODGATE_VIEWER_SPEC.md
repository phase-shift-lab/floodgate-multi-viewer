...
- 1/2/4局表示、盤ごとの対局選択・ピン・反転・拡大・棋譜開閉・最新追従。PCの4局表示は各カードを「盤面左・情報と操作右」のコンパクト構成にし、1440×900で2×2の4局すべてをスクロールせず確認できる。棋譜や評価値を展開した場合と極端に低い画面高では縦スクロールを許容する。1/2局表示とスマートフォンは従来の可読性を保つ。
